...
# The drill GUI's pause menu lists every scenario, with the unported ones shown
# disabled rather than hidden - so the server has to know the whole curriculum,
# not just the ported part.
#
# It cannot work that out for itself. The image ships `scenarios/answers/` and
# nothing else from `scenarios/`, deliberately, so the cards are not in it; and
# hardcoding `01`..`12` in the server is a constant that goes stale in silence the
# day a thirteenth card is written - the menu would simply never show it, with
# nothing failing anywhere.
#
# So the catalogue is GENERATED from the cards, on the laptop, and committed. It
# lands under scenarios/answers/ because that tree is already in the image's
# allow-list, and widening a public image's contents for a menu label is not a
# trade worth making. `--check` keeps it honest: add a card, forget to
# regenerate, and `make -f Makefile.test test` goes red.
CATALOGUE = REPO / "scenarios" / "answers" / "catalogue.json"
CARD_RE = re.compile(r"^(\d{2})-.*\.md$")
CARD_TITLE_RE = re.compile(r"^#\s*\d{2}\s*-\s*(.+?)\s*$", re.MULTILINE)
...
def catalogue() -> str:
    """Every scenario the curriculum has, and whether it is ported yet.

    The title is the card's H1 with its number stripped, so there is exactly one
    place a scenario is named and it is the card itself. `ported` is the presence
    of an answers TOML, which is the same test the grader applies - a scenario is
    "ported" precisely when there is something to grade it with.
    """
    ported = set(answers.available())
    rows = []
    for card in sorted((REPO / "scenarios").glob("*.md")):
        m = CARD_RE.match(card.name)
        if not m:
            continue  # scenarios/README.md
        num = m.group(1)
        heading = CARD_TITLE_RE.search(card.read_text(encoding="utf-8"))
        rows.append(
            {
                "id": num,
                "title": heading.group(1) if heading else num,
                "ported": num in ported,
            }
        )
    return json.dumps(
        {
            "note": "Generated by scripts/gen-answers.py from scenarios/*.md. Do not hand-edit.",
            "scenarios": rows,
        },
        indent=2,
        sort_keys=True,
...
def main(argv: list[str]) -> int:
    ...
    cat = catalogue()
    cat_current = CATALOGUE.read_text(encoding="utf-8") if CATALOGUE.exists() else ""
    ...
    if mode == "--check":
        if cat != cat_current:
            print(
                f"gen-answers: {CATALOGUE.relative_to(REPO)} is STALE - run `make answers-gen`",
                file=sys.stderr,
            )
            return 1
        if out == doc:
    ...
    if cat != cat_current:
        CATALOGUE.write_text(cat, encoding="utf-8")
        print(f"gen-answers: rewrote {CATALOGUE.relative_to(REPO)}")
